...
Public Class MasterSatuan
    Sub KondisiAwal()
        ...
        Call Koneksi()
        Da = New OdbcDataAdapter("Select KODE,SATUAN From satuan", Conn)
        Ds = New DataSet
        Da.Fill(Ds, "satuan")
        GunaDataGridView1.DataSource = Ds.Tables("satuan")
    ...
    Sub NomorOtomatis()
        Call Koneksi()
        Cmd = New OdbcCommand("Select * from satuan where KODE in (select max(KODE) From satuan)", Conn)
        ...
        Else
            ...
            UrutanKode = "ST" + Microsoft.VisualBasic.Right("00" & Hitung, 2)
    ...
    Private Sub GunaButton1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles GunaButton1.Click
        ...
        Else
            ...
            Else
                Call Koneksi()
                Dim InputData As String = "Insert into satuan values ('" & GunaTextBox1.Text & "','" & TBuser.Text & "')"
    ...
    Private Sub GunaDataGridView1_CellContentClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles GunaDataGridView1.CellContentClick
        ...
        If GunaButton1.Text = "TAMBAH" Then
            ...
            Cmd = New OdbcCommand("Select * from satuan where KODE='" & GunaDataGridView1.Item(0, i).Value & "'", Conn)
            Rd = Cmd.ExecuteReader
            Rd.Read()
            If Not Rd.HasRows Then
                GunaButton1.Focus()
            Else
                ...
                GunaTextBox1.Text = Rd.Item("KODE")
                TBuser.Text = Rd.Item("SATUAN")
                TBuser.Focus()
    ...
    Private Sub GunaButton3_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles GunaButton3.Click
        ...
        Else
            ...
            Else
                Call Koneksi()
                Dim UpdateData As String = "Update satuan set SATUAN= '" & TBuser.Text & "' where KODE='" & GunaTextBox1.Text & "'"
    ...
    Private Sub GunaButton4_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles GunaButton4.Click
        ...
        If MessageBox.Show("ANDA YAKIN AKAN MENGHAPUS DATA...?", "", MessageBoxButtons.YesNo) = Windows.Forms.DialogResult.Yes Then
            ...
            Call Koneksi()
            Dim HapusData As String = "Delete from satuan where KODE ='" & GunaTextBox1.Text & " '"
